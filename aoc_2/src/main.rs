...
707	668	1778	1687	2073	1892	62	1139	908	78	1885	800	945	712	57	65
    ...
    println!("checksum is {}", part_1(&input));
    println!("checksum is {}", part_2(&input));
}

fn part_1(input : &str) -> i32 {
    let mut sum = 0;
    for line in input.lines() {
        let tokens = line.split_whitespace().map(|x| x.parse::<i32>().unwrap());
        if tokens.clone().count() <= 0 { continue; }
        sum += tokens.clone().max().unwrap() - tokens.clone().min().unwrap();
    }
    sum
}
...
fn part_2(input : &str) -> i32 {
    let mut sum = 0;
    for line in input.lines() {
        let tokens : Vec<i32> = line.split_whitespace().map(|x| x.parse::<i32>().unwrap()).collect();
        if tokens.len() == 0  { continue; }
        'outer: for (i,val1) in tokens.iter().enumerate() {
            for (j,val2) in tokens.iter().enumerate() {
                if i == j {continue;}
                if val1 % val2 == 0 { sum += val1/val2; break 'outer;}
            }
        }
    }
    sum
